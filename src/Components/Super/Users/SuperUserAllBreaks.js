...
import superUsersStyles from '../../../Styles/Users/SuperUserAllBreaks.module.css'
function SuperUserAllBreaks({...navData}) {
...
<div><div className={superUsersStyles.enter} >Start Pause</div>   {startPause.map((e)=> <div key={Math.random()}> <br></br>{stoPause++}. {e} </div> )}</div> 
  <div><div className={superUsersStyles.enter} >End Pause</div>   {endPause.map((e)=> <div key={Math.random()}> <br></br> {stoEndPause++}. {e}</div> )}</div> 
...
export default SuperUserAllBreaks